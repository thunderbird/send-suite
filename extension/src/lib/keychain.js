...
export default class Keychain {
  constructor(secretKeyB64, nonce) {
    ...
    if (!nonce) {
      console.log(`we're using a hard-coded nonce...`);
      console.log(`but the backend will have/use a different one?`);
    } else {
      console.log(`they passed us a nonce ${nonce}`);
    }
    this._nonce = nonce || "yRCdyQ1EMSA3mo4rqSkuNQ==";
    if (secretKeyB64) {
      this.rawSecret = b64ToArray(secretKeyB64);
    } else {
      console.log(`no secret key passed in, generating some random values`);
  ...
  // called by ownedFile setPassword
